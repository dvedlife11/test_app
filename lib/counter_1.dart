...
class Counter1Screen extends StatefulWidget {
  ...
  @override
  State<Counter1Screen> createState() => _Counter1ScreenState();
}

class _Counter1ScreenState extends State<Counter1Screen> {
  int counter1Count = 0;
  int lifetimeCount = 0;
  int audioCountToday = 0;
  int? dailyTarget;
  bool _autoAddAfterEachListen = false;
  bool _counter1Enabled = true;
  ...
  String _counterUmbrellaSwitchLabel = 'Counter: Select umbrella affirmation';
  ...
  void initState() {
    ...
    _loadCounter1EnabledPreference();
  ...
  Future<void> _loadCounter1EnabledPreference() async {
    final enabled = await repository.getCounter1Enabled();
    if (!mounted) return;
    setState(() {
      _counter1Enabled = enabled;
    });
  }

  Future<void> _setCounter1EnabledPreference(bool enabled) async {
    await repository.setCounter1Enabled(enabled);
    if (!mounted) return;
    setState(() {
      _counter1Enabled = enabled;
    });
  ...
  bool _ensureCounter1Enabled() {
    if (_counter1Enabled) return true;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        duration: Duration(seconds: 4),
        content: Text('Counter 1 is disabled. Enable it to use these actions.'),
      ),
    );
    return false;
  }
  ...
  Future<void> _loadAudioAutoAddPreference() async {
    final enabled = await repository.getAudioSessionAutoAddToA1();
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _setAudioAutoAddPreference(bool enabled) async {
    await repository.setAudioSessionAutoAddToA1(enabled);
    if (!mounted) return;
    setState(() {
      _autoAddAfterEachListen = enabled;
    });
  ...
  Future<void> _loadCounter() async {
    await repository.settleAudioSessionAutoDailyTotalsForCounter1();
    final data = await repository.getCounterData(AppRepository.counter1Count);
    final unappliedPlays =
        await repository.getAudioSessionUnappliedPlaysToday();
    final a1PerSession = await repository.getAudioSessionTrackUsageForIndex(0);
    final audioTodayTotal = unappliedPlays * a1PerSession;

    if (!mounted) return;

    final target =
        await repository.getCounterDailyTarget(AppRepository.counter1Count);

    setState(() {
      counter1Count = data.last24hCount;
      lifetimeCount = data.lifetimeCount;
      audioCountToday = audioTodayTotal;
      dailyTarget = target;
    });
  }

  Future<void> _incrementCounter() async {
    if (!_ensureCounter1Enabled()) return;
  ...
  Future<void> _applyAudioA1Contribution() async {
    if (!_ensureCounter1Enabled()) return;
    final a1PerSession = await repository.getAudioSessionTrackUsageForIndex(0);
    ...
    final unappliedPlays =
        await repository.getAudioSessionUnappliedPlaysToday();
    final amountToAdd = a1PerSession * unappliedPlays;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        content: Text(
          'Affirmation/session: $a1PerSession • New plays: $unappliedPlays • Will add: $amountToAdd',
        ),
      ),
    );
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Affirmation per session: $a1PerSession\nNew plays not yet added: $unappliedPlays\n\nAdd $amountToAdd counts to Counter 1?',
        ),
        actions: [
          TextButton(
    ...
    await repository.addCounterTaps(AppRepository.counter1Count, amountToAdd);
    await repository.markAudioSessionPlaysAppliedToday();
    await _loadCounter();
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        duration: const Duration(seconds: 5),
        content: Text(
          'Added $amountToAdd to Affirmation (Affirmation/session: $a1PerSession × new plays: $unappliedPlays).',
        ),
      ),
    );
  }
                    ...
                    child: ListView(
                      ...
                      children: [
                          ...
                          child: FutureBuilder<String>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              final displayText = text.isEmpty
                                  ? _defaultAffirmationText('affirmation_1')
                                  : text;
                              return Text(
                                '"$displayText"',
                            ...
                            Expanded(
                              child: GestureDetector(
                                onTap: () async {
                                  ...
                                  await _applyAudioA1Contribution();
                                },
                                child: const AppSecondaryButton(
                                  label: '+ Audio',
                                  compact: true,
                        ...
                        Center(
                          child: AppSelectionPillButton(
                            label: _showAffirmationCard
                                ? 'Edit Current Affirmation Below'
                                : 'Modify Affirmation',
                        ...
                        if (_showAffirmationCard) ...[
                          const SizedBox(height: 12),
                          Affirmation1Card(repository: repository),
                        ],
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  GestureDetector(
                                    onTap: () async {
                                      await _setCounter1EnabledPreference(
                                        !_counter1Enabled,
                                      );
                                    },
                                    child: AnimatedContainer(
