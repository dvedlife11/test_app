...
class Affirmation1Card extends StatefulWidget {
  final AppRepository repository;

  const Affirmation1Card({super.key, required this.repository});

  @override
  State<Affirmation1Card> createState() => _Affirmation1CardState();
}

class _Affirmation1CardState extends State<Affirmation1Card> {
  static const String _defaultAffirmation1 = 'Enter affirmation 1';
  ...
  Future<void> _saveTarget() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Counter 1 target saved: $parsed')));
  }
  ...
  Future<void> _editAffirmation() async {
    final current = await widget.repository.getAffirmationText('affirmation_1');
    final controller = TextEditingController(
      text: current.isEmpty ? _defaultAffirmation1 : current,
    );

    final edited = await showAppBottomPopupDialog<String>(
      context: context,
      title: 'Edit affirmation 1',
      content: TextField(
    ...
    if (edited == '__delete__') {
      await widget.repository.saveAffirmationText('affirmation_1', '');
      setState(() {});
      return;
    }
    ...
    if (mode == 'reset') {
      await widget.repository.resetCounterLifetime(
        AppRepository.counter1Count,
        affirmationKeyOverride: 'affirmation_1',
      );
      await widget.repository.saveAffirmationText('affirmation_1', edited);
    } else {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<String>(
      future: widget.repository.getAffirmationText('affirmation_1'),
      builder: (context, textSnapshot) {
        final text =
            (textSnapshot.data == null || textSnapshot.data!.trim().isEmpty)
                ? _defaultAffirmation1
                : textSnapshot.data!.trim();

        return FutureBuilder<CounterData>(
          future: widget.repository.getCounterData(AppRepository.counter1Count),
