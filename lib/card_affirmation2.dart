...
class Affirmation2Card extends StatefulWidget {
  final AppRepository repository;

  const Affirmation2Card({super.key, required this.repository});

  @override
  State<Affirmation2Card> createState() => _Affirmation2CardState();
}

class _Affirmation2CardState extends State<Affirmation2Card> {
  static const String _defaultAffirmation2 = 'Enter affrimation 2';
  ...
  Future<void> _saveTarget() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Counter 2 target saved: $parsed')));
  }
  ...
  Future<void> _editAffirmation() async {
    final current = await widget.repository.getAffirmationText('affirmation_2');
    final controller = TextEditingController(
      text: current.isEmpty ? _defaultAffirmation2 : current,
    );

    final edited = await showAppBottomPopupDialog<String>(
      context: context,
      title: 'Edit affirmation 2',
      content: TextField(
    ...
    if (edited == '__delete__') {
      await widget.repository.saveAffirmationText('affirmation_2', '');
      setState(() {});
      return;
    }
    ...
    if (mode == 'reset') {
      await widget.repository.resetCounterLifetime(
        AppRepository.counter2Count,
        affirmationKeyOverride: 'affirmation_2',
      );
      await widget.repository.saveAffirmationText('affirmation_2', edited);
    } else {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<String>(
      future: widget.repository.getAffirmationText('affirmation_2'),
      builder: (context, textSnapshot) {
        final text =
            (textSnapshot.data == null || textSnapshot.data!.trim().isEmpty)
                ? _defaultAffirmation2
                : textSnapshot.data!.trim();

        return FutureBuilder<CounterData>(
          future: widget.repository.getCounterData(AppRepository.counter2Count),
